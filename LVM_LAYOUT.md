Learn more: [An Introduction to LVM Concepts, Terminology, and Operations](https://www.digitalocean.com/community/tutorials/an-introduction-to-lvm-concepts-terminology-and-operations)
## Current mapping
```text
nvme0n1                   931.5G disk             
├─nvme0n1p1                   1G part vfat        /boot/efi
├─nvme0n1p2                   4G part swap        [SWAP]
└─nvme0n1p3               926.5G part LVM2_member 
  ├─leela--vg0-leela--lv1    20G lvm  ext4        /
  ├─leela--vg0-leela--lv2   425G lvm  ext4        /home
  ├─leela--vg0-leela--lv3    50G lvm  ext4        /var
  └─leela--vg0-leela--lv4     2G lvm  ext4        /var/log
```
## Proposed mapping
```text
nvme0n1                   931.5G disk             
├─nvme0n1p1                   1G part vfat        /boot/efi
├─nvme0n1p2                   4G part swap        [SWAP]
└─nvme0n1p3               926.5G part LVM2_member
  ├─leela--vg0-root          20G lvm  ext4        /
  ├─leela--vg0-home         425G lvm  ext4        /home
  ├─leela--vg0-var           50G lvm  ext4        /var
  └─leela--vg0-log            2G lvm  ext4        /var/log
```
